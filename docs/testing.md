...
- `test_workday_filters.py` — Workday facet discovery and location resolution against trimmed fixtures from seven real tenants. Pins that nothing is keyed by facet *name*: `locationCountry` (Adobe), `locationHierarchy1` (NVIDIA), a `CF_-_REC_-_LRV_-_…` custom field (Salesforce), and no country facet at all (ServiceTitan) must all resolve "Canada"; a country prefers the single country value over many city values; a tenant without a country facet ORs every matching city
- `test_amazon_jobs.py` — amazon.jobs: URL detection (postings/search, and that the retail `amazon.ca`/`amazon.com` storefront is never captured), category slugification, pay-band extraction (Canadian band, cents dropped only when both ends have none so hourly bands survive, multi-location dedupe, absent band is empty not an error), render
- `test_apple_jobs.py` — Apple: URL detection (details/search, locale extraction), `{slug}-{CODE}` location construction, locale fallback, hydration parsing (double-decode round trip, escaped quotes inside content, missing marker and malformed payload return None), render
- `test_meta_careers.py` — Meta: URL detection (job/index, apex host), `search_input` completeness (Meta needs its full key set), balanced-JSON extraction from `data-sjs` blobs (braces and escaped quotes inside strings), `doc_id` discovery pattern against Meta's real module shape; rate-limit handling (a throttled `HTTP 200` must back off rather than trigger a bundle scan, and must stay distinguishable from a rotated `doc_id`)
- `test_uber_jobs.py` — Uber: URL detection (global/regional careers paths, `jobs.uber.com`, rejection of consumer uber.com pages), Long-triple count unwrapping, location param building, location splitting, all-null locations labelled "Not specified" rather than dropped, render
- `test_job_board_routing.py` — `fetch()` routing for all six big-tech boards, with negative cases: retail Amazon, consumer Uber, unrelated career pages, Workday's own existing route, and incomplete paths must all fall through untouched; a selected posting wins over the board on the same host
- `test_discovery_payload.py` — SPA discovery payload handling: anti-hijacking guards, NDJSON, length-prefixed chunk streams, JSON nested inside a string; why Amazon's one-key facet dicts are not a record set while Workday's 334 video-player labels are; head-window value sampling
- `test_discovery_ranking.py` — The two opposed live cases, both reproduced: Netflix (branding outscores the listing, the query hint picks the listing) and Workday (the lookup table outnumbers the postings, score picks the postings); hint extraction rejecting analytics blobs; refusal to answer; the navigation document penalized but not excluded
- `test_discovery_oracle.py` — The five match rules with their tuned thresholds, including the two added after the minimizer silently broke a plan: the upper record bound (a dropped filter returns *more* records) and the 50% content overlap (Apple returns the same 20 rows with or without a query)

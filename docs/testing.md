...
- `test_meta_careers.py` — Meta: URL detection (job/index, apex host), `search_input` completeness (Meta needs its full key set), balanced-JSON extraction from `data-sjs` blobs (braces and escaped quotes inside strings), `doc_id` discovery pattern against Meta's real module shape

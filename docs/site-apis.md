...
**`/compare/<category>/` is the whole value and the plain HTML path returned none of it.** The tool is an Alpine.js app whose empty state — "No products found.", "No brand found.", "Accordion Title" — renders as a comparison board with nothing on it rather than as a failed read. The dataset is in fact inline in the page as `window.vwProducts`: over 420 listings / 350 models and still growing, 91 fields each, carrying Vacuum Wars' **own lab measurements** (suction kPa, airflow cfm, carpet deep clean, crevice pickup, flattened pet-hair pickup, seven-inch hair tangle, mopping water, ft² per charge) and the five star scores those roll up into. Nothing else on the site publishes this in machine-readable form. It is parsed here and the shell is discarded — content analysis, not a blocking problem, so none of it is wafer's.

Four things about that dataset decide whether the output is honest:

- **One request gets everything.** The tool paginates client-side over the array (`vwProducts.length / this.itemsPerPage`), so `/compare/robot-vacuums/page/2/` re-serves the same payload. Never fetch it as though it held more.
- **Tested and merely listed are different populations.** Roughly half the models have lab results; the rest are spec sheets for robots that were never tested. They are counted separately, ranked separately, and an absent measurement renders `-`, because "we never tested this" and "it scored nothing" must not look alike.
- **Colour variants are the same robot.** Around a sixth of the listings are a second shell of a model already present. They collapse only when brand, base name **and every measurement** match, and the row then says how many listings it stands for. Where the variants' prices disagree — the X60 Max Ultra Complete is $1,614.99 in black and $1,699.99 in white — the row says **"from"** and quotes the cheaper, the same rule ui.com's `minDisplay*` fields get.
- **Prices are the tool's last cached Amazon figure** and drift from the live figure the article pages show ($599.00 vs $598.99 on the T90 Pro Omni). They move independently of the scores and are labelled as cached.

**There is no JSON endpoint, and looking for one is settled.** The comparison app's bundle is byte-identical on both hosts and makes exactly one network call, `admin-ajax.php` for the feedback form. The dataset is only ever inlined into the HTML, so a >1.3 MB page fetch *is* the cheapest available read; there is nothing lighter to find. `robots.txt` disallows `/compare/` but then explicitly re-allows `/compare/robot-vacuums` and `/compare/robot-vacuums/`, so the one path this module reads is the one the site deliberately opened, and GPTBot/CCBot/Google-Extended are all allowed too. Because every fetch costs the site a full render of that page, `vacuumwars_limiter` throttles the domain at 2s despite no block ever having been observed — politeness, not evasion.

**Counts here are approximate on purpose.** The array is bigger than any single capture this repo can take through its own fetch tool (the 250k-token ceiling is ~1M characters and the payload exceeds it), and `Range` is ignored by both hosts, so the tail has never been read from a client. The server-side parse has no such ceiling and sees the whole array; only measurement from outside is bounded. Do not hardcode a total, and treat any figure in this file as a floor that was true when written.

`compare.vacuumwars.com` is the tool's own front end and serves the **same dataset** — 91 fields, byte-identical scores and prices for every slug present in both — with the array starting ~2 KB into the document instead of ~285 KB, so it is much the cheaper source when a caller has that URL. Every route on that host is the tool, so `is_compare_url()` decides on host there and on path only for the main site. Its `/embed/?product1=&product2=` route answers **404 while serving the real app**, the same reverse-soft-404 as the main site's `-vs-` URLs, so fetchaller still errors on it.

The comparison tool is **robot vacuums only** — `/compare/cordless-vacuums/` is a hard 404, and no other category page carries `vwProducts`. Cordless, upright and carpet-cleaner data lives in the article prose and its tables, which render fine. Extraction is gated on the `/compare/` path *and* the global being present: a review page carrying the same global must never be thrown away and re-rendered as a spec table. On a `/compare/` path with no readable dataset the output says so, rather than falling through to the empty state.

Note (**unverified, do not build on it**): a head-to-head URL such as `/compare/robot-vacuums/<slug>-vs-<slug>/` answers **HTTP 404 while serving the real compare template** (correct title, self-referential canonical), so fetchaller currently errors on it. Whether that body carries `vwProducts` was never confirmed. It buys nothing anyway — the plain compare URL already returns every model — so the fix, if wanted, needs the body checked first.

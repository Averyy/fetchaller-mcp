...
## vacuumwars.com (comparison tool dataset, leaderboard cards)

`src/fetchaller/content/vacuumwars.py`. Independent lab testing of robot, cordless, upright and carpet-cleaning vacuums. Anonymous throughout; there is **no credentialed path** to any part of the site.

**`/compare/robot-vacuums/` is the whole value and the plain HTML path returned none of it.** The tool is an Alpine.js app whose empty state — "No products found.", "No brand found.", "Accordion Title" — renders as a comparison board with nothing on it rather than as a failed read. The dataset is in fact inline in the page as `window.vwProducts`: 91 fields per listing, carrying Vacuum Wars' **own lab measurements** (suction kPa, airflow cfm, carpet deep clean, crevice pickup, flattened pet-hair pickup, seven-inch hair tangle, mopping water, ft² per charge) and the five star scores those roll up into. Nothing else on the site publishes this in machine-readable form. It is parsed here and the shell is discarded — content analysis, not a blocking problem, so none of it is wafer's.

Four things about that dataset decide whether the output is honest:

- **One request gets everything, and there is no second request to make.** The tool paginates client-side over the array (`vwProducts.length / this.itemsPerPage`), and WordPress has no route behind it: `/compare/robot-vacuums/page/2/` is a hard 404 with a 306 KB error body, carrying no `vwProducts` at all. Paging for more returns nothing, not a duplicate.
- **Tested and merely listed are different populations, and the tested one is small.** On 2026-09-11 the array held 894 listings, of which 222 carried any lab result and 205 a star score; after variant collapsing that is 752 rows, 187 of them tested. So roughly three quarters of the catalogue is a spec sheet for a robot the lab never touched. They are counted separately, ranked separately, and an absent measurement renders `-`, because "we never tested this" and "it scored nothing" must not look alike.
- **Colour variants are the same robot, and only the specs can prove it.** About a sixth of the listings are a second shell of a model already present, and they collapse only when brand, base name, every measurement **and every spec the table prints** match. Matching on measurements alone is the trap: three quarters of the catalogue has a null for every measurement, so the signature goes degenerate there and the rule decays to brand-plus-name. That merged an "OKP L1" rated 1400 Pa with obstacle avoidance into an "OKP L1 (White)" rated 4000 Pa without it, and an "Airrobo T30+ (White)" into the "(60-Day Dock)" SKU costing three times as much — one member's specs printed beside the other member's price, the exact substitution "from" exists to prevent. Because the site types the same field as `4000` on one listing and `"4000"` on the next for about 40% of the numeric specs, the comparison is on the rendered value, not the raw one. A trailing parenthetical is dropped **only where dropping it is what merged the rows** — more than one listing, under more than one name. A row that stands alone keeps the name the site gave it: "(No self-empty station)", "(No AutoEmpty Dock)", "(Amazon Exclusive)", "(Hub 2 required for Matter)" and bare model numbers like "(7550)" are not colours, and "Eufy L60 (No self-empty station)" sits in the catalogue right beside "Eufy L60 with Self Empty Station". Stripping by default renamed 158 rows, 16 of them load-bearing. Where the merged variants' prices disagree the row says **"from"** and quotes the cheaper, the same rule ui.com's `minDisplay*` fields get — and it says "from" when a member carries no cached price at all, since a bare figure on a two-listing row claims both cost it. Two rows still share a name where the site itself lists one twice (`3i G10+`, `Lefant M210 Pro Omni`); their specs differ visibly and there is no disambiguator in the data.
- **Prices are the tool's last cached Amazon figure** and drift from the live figure the article pages show. They move independently of the scores and are labelled as cached — and that caveat is printed **before** the tables, not after, because at the catalogue's real size the full render is ~30k tokens and the fetch tool's 25k default cut the closing paragraph off. For the same reason the spec table runs tested models first, so a budget-truncated read keeps the lab results and loses only manufacturer specs; it carries no score column, since the ranked table above already has one for every row that has a score; and the header prints a per-brand index of the untested tail. That tail is sorted by brand, so a cut takes whole brands off the end of the alphabet — without the index a truncated row reads as a model the site does not list.

**Rank means rank.** Sixteen models carry individual test results but no overall score. They belong in the tested table, but numbering them would make a dataset position read as a placing, so their rank cell is `-` and the table states how many trailing rows are unordered. A genuine score of `0` is a score and still ranks.

**There is no JSON endpoint, and looking for one is settled.** The comparison app's bundle is byte-identical on both hosts and makes exactly one network call — `fetch("/wp-admin/admin-ajax.php")` with `action: vacuumwars_send_message`, the feedback form. `vacuumwars_send_message` is the only action string in the bundle. The dataset is only ever inlined into the HTML, so the 2.6 MB page fetch *is* the cheapest available read; there is nothing lighter to find. `robots.txt` disallows `/compare/` but then explicitly re-allows `/compare/robot-vacuums` and `/compare/robot-vacuums/`, so the one path this module reads is the one the site deliberately opened, and GPTBot/CCBot/Google-Extended are all allowed too. Because every fetch costs the site a full render of that page, `vacuumwars_limiter` throttles the domain at 2s despite no block ever having been observed — politeness, not evasion.

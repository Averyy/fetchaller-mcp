...
- **Tested and merely listed are different populations.** 167 of the 245 models have lab results; the other 78 are spec sheets for robots that were never tested. They are counted separately, ranked separately, and an absent measurement renders `-`, because "we never tested this" and "it scored nothing" must not look alike.
- **Colour variants are the same robot.** 62 of the listings are a second shell of a model already present. They collapse only when brand, base name **and every measurement** match, and the row then says how many listings it stands for. Where the variants' prices disagree — the X60 Max Ultra Complete is $1,614.99 in black and $1,699.99 in white — the row says **"from"** and quotes the cheaper, the same rule ui.com's `minDisplay*` fields get.
